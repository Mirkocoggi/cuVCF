...
def check_lengths(d, name):
    """Controlla che tutte le colonne in `d` abbiano la stessa lunghezza."""
    lengths = {k: len(v) for k, v in d.items()}
    uniq    = set(lengths.values())
    if len(uniq) == 1:
        print(f"{name}: OK – {uniq.pop():,} righe uniformi")
    else:
        print(f"{name}: colonne di lunghezza diversa!")
        for col, n in lengths.items():
            print(f"  {col:<25}: {n:,}")
...
def write_dict_to_csv_batched(data_dict, out_csv, chunk_size):
    """
    Scrive un dizionario di colonne (NumPy/CuPy/list) in CSV senza mai
    tenere l’intero DataFrame sulla GPU.
    """
    # header una sola volta
    header = ",".join(map(str, data_dict.keys())) + "\n"
    ...
    for start in range(0, n_rows, chunk_size):
        ...
        block = {k: v[start:end] for k, v in data_dict.items()}
        gdf   = cudf.DataFrame(block)
        # append senza header
        with open(out_csv, "a") as f:
            f.write(gdf.to_csv(index=False, header=False))
        del gdf, block
        free_gpu()

def chunk_iter(*dicts, chunk):
    """Yield sottodizionari sincronizzati su data1, data2, data3, data4."""
    n = len(next(iter(dicts[0].values())))
    for s in range(0, n, chunk):
        e = min(s + chunk, n)
        yield tuple({k: v[s:e] for k, v in d.items()} for d in dicts)

def estimate_dict_size(d):
    return sum(sys.getsizeof(v) for v in d.values()) // (1024**2)
...
res.run("data/chrx_AAAAAA.vcf", 16)
...
print(f"data1 approx size: {estimate_dict_size(data1)} MB")
print(f"data2 approx size: {estimate_dict_size(data2)} MB")
print(f"data3 approx size: {estimate_dict_size(data3)} MB")
print(f"data4 approx size: {estimate_dict_size(data4)} MB")

check_lengths(data1, "data1")
check_lengths(data2, "data2")
check_lengths(data3, "data3")
check_lengths(data4, "data4")
